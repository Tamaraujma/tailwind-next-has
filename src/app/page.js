...
function Card(props) {
  return (
    <li className="group overflow-hidden rounded-xl bg-white p-4 shadow-md ring-1 ring-black/5 transition hover:-translate-y-px hover:shadow-lg">
      {props.image && <img src={props.image} alt="" className="mb-4 group-has-[h2]:p-0" />}
      <div className="grid gap-2">
        {props.title && <h2 className="group-not-has-[img]:text-2xl text-xl font-medium">{props.title}</h2>}
        <p className="text-slate-700 group-not-has-[h2,img]:text-2xl/[1.4]">{props.text}</p>
      </div>
    </li>
  );
}
